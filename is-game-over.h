...
bool traverse_board(int hexes[SIZE][SIZE], bool visited[SIZE][SIZE], int x, int y, int size, int current_player, int opposite_player) {
    ...
    // ignore this symbol
    if (x < OUT_OF_BOUNDS_COORDINATE || x >= size || y < OUT_OF_BOUNDS_COORDINATE || y >= size || visited[x][y] || hexes[x][y] != current_player) return false;
    ...
    return (traverse_board(hexes, visited, x, y - NEIGHBOR_DISTANCE, size, current_player, opposite_player) ||
            traverse_board(hexes, visited, x, y + NEIGHBOR_DISTANCE, size, current_player, opposite_player) ||
            traverse_board(hexes, visited, x + NEIGHBOR_DISTANCE, y, size, current_player, opposite_player) ||
            traverse_board(hexes, visited, x - NEIGHBOR_DISTANCE, y, size, current_player, opposite_player) ||
            traverse_board(hexes, visited, x + NEIGHBOR_DISTANCE, y + NEIGHBOR_DISTANCE, size, current_player, opposite_player) ||
            traverse_board(hexes, visited, x - NEIGHBOR_DISTANCE, y - NEIGHBOR_DISTANCE, size, current_player, opposite_player));

}

// this function checks the pawns on the board edges and looks for the connections between board edges
bool launch_traverse_on_board_edge(int hexes[SIZE][SIZE], bool visited[SIZE][SIZE], int x, int y, int current_player, int opposite_player, const int size, bool* winner) {

    // if the symbol is a pawn and if the connection between edges made from the same kind of pawns is found ...
    if (hexes[x][y] == current_player && traverse_board(hexes, visited, x, y, size, current_player, opposite_player)) {
...
bool find_connection(int hexes[SIZE][SIZE], const int size, bool* winner, bool visited_blue[SIZE][SIZE], bool visited_red[SIZE][SIZE]) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (launch_traverse_on_board_edge(hexes, visited_red, edge_symbol_mark, i, red_pawn_symbol, blue_pawn_symbol, size, winner)) {
            return true;
        }

        // if an 'b' was found along the edge and the connection to the opposite edge is found too, then the game is over
        if (launch_traverse_on_board_edge(hexes, visited_blue, i, edge_symbol_mark, blue_pawn_symbol, red_pawn_symbol, size, winner)) {
            return true;
        }
...
bool check_is_game_over(int hexes[SIZE][SIZE], const int* blue_pawns_counter, const int* red_pawns_counter, const int* number_of_hexes, bool* winner, bool print_the_result = false) {
    ...
    bool result = find_connection(hexes, size, winner, visited_blue, visited_red);
